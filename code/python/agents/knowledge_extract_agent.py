...
你是一个面向企业安全运营的知识抽取引擎。给定一段文本，请提取其中的：
...
注意:
- 通用实体类型: Person, Organization, Location, Product, Technology, Concept, Event, Time
- 安全实体类型: ThreatActor, Malware, Vulnerability, Asset, AttackTechnique, IOC, SecurityControl
- 通用关系类型: belongs_to, works_at, located_in, developed_by, related_to, part_of, uses, depends_on
- 安全关系类型: exploits, targets, affects, communicates_with, mitigates, attributed_to, indicates
...
class KnowledgeExtractAgent:
    ...
    async def _extract_from_text(self, text: str, source_id: str) -> ExtractionResult:
        if self.llm is None:
            return await self._extract_security_rules(text, source_id)
        messages = [
    ...
    async def _extract_security_rules(self, text: str, source_id: str) -> ExtractionResult:
        """Build an auditable security subgraph without calling an LLM."""
        analysis = await self.security_agent.analyze(text, source=source_id)
        event_name = f"SecurityEvent:{source_id}"
        entities = [
            Entity(
                name=event_name,
                type="Event",
        ...
        for indicator in analysis.indicators:
            entities.append(Entity(
                ...
                type="IOC",
                description=f"{indicator.type} indicator",
                properties={"indicator_type": indicator.type},
            ))
            relations.append(Relation(
                head=event_name,
                relation="observed_ioc",
                tail=indicator.value,
                confidence=indicator.confidence,
            ))

        events: list[KnowledgeEvent] = []
        for technique in analysis.techniques:
            technique_name = f"{technique.technique_id} {technique.name}"
            entities.append(Entity(
                name=technique_name,
                type="AttackTechnique",
                description=f"{technique.tactic}; evidence: {technique.evidence}",
            ))
            relations.append(Relation(
                head=event_name,
            ...
            events.append(KnowledgeEvent(
                trigger=technique.evidence,
                type=technique.tactic,
                participants=[event_name, technique_name],
            ))
