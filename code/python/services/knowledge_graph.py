...
class KnowledgeGraphService:
    ...
    async def add_relation(self, relation: Relation, source: str = "") -> None:
        ...
        if not self._driver:
            return
        rel_type = re.sub(r"[^A-Z0-9_]", "_", relation.relation.upper().replace(" ", "_"))[:64]
    ...
    async def execute_read_cypher(self, cypher: str, params: dict | None = None) -> list[dict]:
        """执行受限只读查询，防止模型生成的 Cypher 修改或导出数据库。"""
        normalized = " ".join(cypher.strip().lower().split())
        blocked = (
            " create ", " merge ", " delete ", " detach ", " set ", " remove ",
            " drop ", " load csv", " apoc.", " dbms.", " call ", ";",
        )
        padded = f" {normalized} "
        if not normalized.startswith(("match ", "optional match ")):
            raise ValueError("仅允许 MATCH/OPTIONAL MATCH 只读查询")
        if " return " not in padded or any(token in padded for token in blocked):
            raise ValueError("查询包含非只读语句")
